...
| **Cluster sizes** | • **Cluster 0 (Single-Order Recent):** 53,255 customers (55.42%)<br>• **Cluster 1 (Single-Order Lapsed):** 39,836 customers (41.45%)<br>• **Cluster 2 (High-Value Repeat VIPs):** 3,005 customers (3.13%) |
...
* **Why This Evidence is Weak:**
  ...
  * Both $k=3$ and $k=4$ yield silhouette scores around ~0.41–0.48, signaling weak natural separation. This happens because of the ~97% single-buyer rate.
...
* **WITH Scaling (`StandardScaler`):**  
  ...
  * **Impact:** Feature scaling allowed K-Means to isolate the rare ~3.1% repeat buyers into a dedicated segment (**Cluster 2**), characterized by an average of 2.11 orders and double the mean spend.
...
* **Survival:** Yes. In the Jupyter Notebook, cluster assignments proved exceptionally stable across different random seeds (10, 100, 2024) and 80% data subsamples, maintaining an **Adjusted Rand Index (ARI) > 0.990**. Centroid positions remained consistent across iterations.
...
| :---: | :--- | :---: | :--- |
| **0** | **Single-Order Recent** | 53,255 (55.42%) | Recency ~177d, Spend ~$160, Orders = 1.00. Active single-purchase buyers. |
| **1** | **Single-Order Lapsed** | 39,836 (41.45%) | Recency ~437d, Spend ~$158, Orders = 1.00. Dormant buyers over a year out. |
| **2** | **High-Value Repeat VIPs** | 3,005 (3.13%) | Recency ~268d, Spend ~$326, Orders = 2.11. Core multi-order, high-Customer Lifetime Value buyers. |
...
- Features used and why (and what you did about frequency / missing values):
  ...
  - Frequency / Missing Values Strategy: ~97% of Olist customers buy only once. Features were normalized using `StandardScaler` ($\mu=0, \sigma=1$) so the rare ~3% repeat buyers formed a distinct cluster rather than being erased by the scale of recency/monetary. NaNs in transactional attributes were $<0.1\%$ and removed via `.dropna()`.
...
- Silhouette score, cluster sizes:
  ...
  - Cluster 0: **53,255** customers (55.42%)
  - Cluster 1: **39,836** customers (41.45%)
  - Cluster 2: **3,005** customers (3.13%)
...
- For each cluster: the 2-3 defining features and a one-line persona.
  ...
  - **Cluster 1:** High recency (~437 days), moderate spend (~$158), order count = 1.00.  
    *Persona:* "Dormant, single-order lapsed shoppers at high risk of permanent churn."
...
- One concrete action per segment a team could take:
  ...
  - **Cluster 1 (Lapsed Single-Order):** Deploy low-cost quarterly win-back email discounts while suppressing from paid ad retargeting.
